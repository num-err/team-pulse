...
- [x] **3. Webhook idempotency** — natural `dedup_key` per source + DB `UNIQUE` constraint + upsert with `ignore_duplicates=True`. Migration `0001_activity_events_dedup_key.sql` **confirmed applied against live Supabase 2026-07-25** (column + unique constraint both verified via SQL Editor) — see Database → Migrations.
...
- `/dashboard` → (`app/dashboard/page.tsx`) — team-digest-first live client component:
  ...
  - **Per-actor card hierarchy (polished 2026-07-25):** health state is the visual hero of each card, not one badge among several. Order top to bottom: a small muted avatar+name identity line → `HealthHero` (`components/health-badge.tsx`) — a bigger icon chip + bold colored state label (green Healthy / amber Thrashing / red Silent & stuck / gray Idle), pulsing only for Thrashing/Silent & stuck so calm states stay calm → the AI summary (the story) → a divider → event-count and `SourceIcons` (`components/source-icons.tsx`) demoted to small muted text next to the Slack button. Each card also gets a thin colored accent bar down its left edge (`healthAccentColor()`, same file) matching the health state, so a whole grid of cards is scannable by color alone without reading text. The original compact `HealthBadge` pill still exists in the same file for anywhere a smaller treatment is needed, but `ActorCard` no longer uses it. Explicitly out of scope for this pass: polling/auto-refresh, filtering, deep-links — this was a visual hierarchy polish only, not the "interactive upgrade."
...
**Disk space resolved (confirmed 2026-07-25, later same day).** The `163MB free / 100% full` state noted below was cleaned up — `df -h /` now shows **22GB free out of 228GB (44% used)**. `next dev`/`next build` can be trusted again; no more `ENOSPC` risk. Leaving the original note below for context on what the symptom looked like if it recurs.

Original note: while polishing the dashboard's `ActorCard` earlier the same day, `df -h /` showed **163MB free out of 228GB (100% full)**. `next dev` itself failed outright with `ENOSPC` trying to write its webpack cache (`.next/cache/webpack/server-development/index.pack.gz_`) — this blocks local frontend development entirely, not just the `.next`-corruption symptom above. A Playwright/Chromium install attempted for visual verification also failed with `ENOSPC` mid-download. Likely offenders identified at the time: `~/Library/Caches` 7.9GB, `~/Library/Application Support` 21GB (real app data, not a blind-delete candidate), Docker Desktop 3.9GB (reclaimable via `docker system prune`), `~/Downloads` 2.3GB, stale `node_modules` in other local projects (`squad-dashboard` 760MB, `Trader/swingbot` 513MB, both regenerable via `npm install`).
...
- `0001_activity_events_dedup_key.sql` — adds the `dedup_key` column + `UNIQUE` constraint above. **✅ Applied to live Supabase 2026-07-25** — verified via SQL Editor: `activity_events_dedup_key_unique` present with `contype='u'`, and `dedup_key` column present (`text`, nullable). Existing rows got `dedup_key = NULL`, which is safe — Postgres `UNIQUE` allows unlimited `NULL`s, no backfill was needed. Before this ran, `on_conflict="dedup_key"` upserts would have failed at the Postgres level (`there is no unique or exclusion constraint matching the ON CONFLICT specification`), 500ing every webhook delivery — that risk is now closed.
- `0002_job_runs.sql` — creates the `job_runs` table (see Scheduler → Run persistence). **✅ Applied to live Supabase 2026-07-25** — verified via SQL Editor: table exists with all 5 columns (`id, job_name, ran_at, result, inserted_at`) and the `job_runs_job_name_ran_at_idx` index is present. Before this ran, `record_run()`/`get_last_run()` calls would have failed (caught and logged, wouldn't crash the process), but `/scheduler/status` and `/notion/status` would have silently reported `None`/empty and the missed-run catch-up check wouldn't have worked — that risk is now closed.
- `0003_actor_aliases.sql` — creates the `actor_aliases` table (see Actor Identity Mapping section below). **Safe to defer** — unlike the two above, nothing breaks or degrades before this runs: `resolve_actor()` catches the missing-table error, logs it, and passes every raw actor string through unchanged, exactly like the "no alias configured" case. **Status as of 2026-07-25: unconfirmed** — the SQL was handed to the user to run by hand in the Supabase SQL editor (no results reported back to this session yet); do not assume applied or not-applied until re-verified.

**Re-verification session (2026-07-25).** This coding session has no live connection to Supabase (no MCP/API/credentials) — migration status can only be confirmed by the user running SQL in the Supabase SQL editor and reporting results back. A read-only verification query (checks `information_schema.columns`/`pg_constraint`/`pg_indexes` for all three migrations' objects, no writes) was written for this purpose but not yet run/reported for this pass. While preparing it, one correction surfaced to the failure-mode claims above: **0002 missing does NOT 500 anything** — `record_run()`/`get_last_run()` (`job_runs.py`) both catch and swallow every exception, same graceful-degradation pattern as 0003's `resolve_actor()`. The real symptom if `job_runs` is ever missing is subtler than "status endpoints report None": `catch_up_if_missed()` (`scheduler.py`) calls `get_last_run()` to decide whether today's digest already ran — if that always returns `None`, every process restart past `DIGEST_CRON_HOUR` will conclude no run happened today and **re-send the full team digest to Slack**, once per restart. Only `0001` (missing `dedup_key`/constraint) is a hard 500-on-every-webhook failure, because `routes/webhooks/{github,linear,figma}.py`'s upsert calls have no try/except around them.
...
- Dashboard renders a prominent alert card per `attention[]` entry above everything else, plus a `HealthHero` (state as the visual hero, not a small badge — see Frontend section) on every actor card
